...
BRANCH=cmake
BUILD_SPEC="libsettings@${BRANCH} target=${SPACK_TARGET}"
...
spack spec ${BUILD_SPEC} > /tmp/libsettings_spec_${BRANCH}

# Extracts the exact spec of a dependency then returns the install directory
function location() {
    local spec=$(grep "\^${1}@" /tmp/libsettings_spec_${BRANCH} | sed 's/\s.*^//g')
